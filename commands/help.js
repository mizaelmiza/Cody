    ...
    async run ({message, args, prefix}, t) {
        ...
        if(args[0] && t(`help:${args[0].toLowerCase()}.desc`) !== `${args[0].toLowerCase()}.desc`) {
          ...
          var embed = new this.client.Discord.RichEmbed()
            ...
            .addField(t('comandos:help.aliases'), comandos.filter(command => command.name === cmdName).length > 0 ? comandos.filter(command => command.name === cmdName)[0].aliases.map(aliase => prefix + aliases).join('\n') : t('comandos:help.notHaveAliases'))
